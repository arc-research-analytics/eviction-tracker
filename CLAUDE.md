...
- **CountyTrends** - Manages trend visualization drawer with three tabs: County (per-county lines), Region (5-county aggregate), and City (city-level data from `evictions-city` table)
...
Geography configuration is defined in `DataLoader.geographyConfig` and `LayerManager.geographyConfig`.

A **city** layer configuration also exists in `LayerManager.geographyConfig` (using `region_cities.geojson`, ID field: `ShortLabel`), used internally for city-level trend data but not exposed as a map geography toggle.
...
The map legend is interactive. Users can drag or click on the legend gradient bar to set a `filterThreshold` on `LayerManager`. Features with a `displayvalue` below the threshold are visually dimmed, and hover/tooltip interactions are suppressed for those features (`InteractionManager._isFilteredOut()`). A threshold of 0 means no filtering.
...
- Map rotation is disabled (`dragRotate` and `touchZoomRotate` rotation both disabled in `MapManager`).
...
The slider `START_DATE` is set to `2019-01` in `config.template.js`.
...
2. Serving with VS Code Live Server **on port 5500** — the Mapbox `evictions` token is URL-restricted to `localhost:5500`. Other ports will cause Mapbox to reject the token.
3. The app automatically detects localhost and loads `config.dev.js` instead of `config.js`

## Deployment

The app deploys to GitHub Pages automatically via `.github/workflows/deploy.yml` on every push to `main`. The workflow generates `js/config.js` from GitHub repository secrets (`SUPABASE_URL`, `SUPABASE_ANON_KEY`, `MAPBOX_ACCESS_TOKEN`) at deploy time.

`js/config.js` is also tracked in git locally (added intentionally in commit `6b062c6`) to hold the current production values, but the CI/CD workflow overwrites it from secrets during deployment. `js/config.dev.js` is gitignored and holds local credentials.
...
- `evictions-county` - County-level aggregated data by month (used by CountyTrends County tab)
- `evictions-city` - City-level aggregated data by month (used by CountyTrends City tab); key field: `city_id`
...
- `data/` - GeoJSON files for map boundaries
  ...
  - `region_boundaries.geojson` - County boundary lines (used for non-tract geography)
  - `region_boundaries_tract_simp.geojson` - Simplified county boundaries used when tract geography is active
  - `region_cities.geojson` - City boundary polygons (ID field: `ShortLabel`)
